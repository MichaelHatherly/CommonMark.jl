...
@enum(ColorMode, RESET, COLORS_16, COLORS_256, COLORS_24BIT)
...
Base.inv(c::Crayon) = Crayon(
    inv(c.fg),
    inv(c.bg),
    ANSIStyle(), # no point taking inverse of reset,
    inv(c.bold),
    inv(c.faint),
    inv(c.italics),
    inv(c.underline),
...
function _print(io::IO, c::Crayon)
    ...
    for (col, num) in ((c.fg, 30), (c.bg, 40))
        if col.active
            ...
            col.style == COLORS_24BIT &&
                print(io, num + 8, ";2;", red(col), ";", green(col), ";", blue(col))
        end
    end
