@testitem "json_writer" tags = [:writers, :json] setup = [Utilities] begin
    ...
    function pretty_json(text, parser = p)
        ast = parser(text)
        doc = JSON.parse(json(ast; dicttype = OrderedDict); dicttype = OrderedDict)
        sprint(JSON.print, doc, 2)
    end
    ...
    function roundtrip(text, to = "gfm", parser = p)
        ast = parser(text)
        output = json(ast)
        String(read(pipeline(`echo $output`, `$(pandoc_jll.pandoc()) -f json -t $to`)))
    end
